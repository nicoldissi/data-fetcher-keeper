...
export function EnergyFlowChartDark({
  ...
  return <Card className={cn("overflow-hidden backdrop-blur-sm bg-white/90 border-0 shadow-md h-full", className)}>
      <CardHeader className="pb-2">
        <CardTitle className="text-lg font-medium flex items-center justify-between">
          ...
          <div className="bg-muted inline-flex items-center rounded-md p-1">
            <Toggle pressed={viewMode === 'realtime'} onPressedChange={() => setViewMode('realtime')} variant="outline" size="sm" className="px-3 data-[state=on]:bg-background">
              <Clock className="h-4 w-4 mr-2" />
              <span>Temps réel</span>
            </Toggle>
            <Toggle pressed={viewMode === 'daily'} onPressedChange={() => setViewMode('daily')} variant="outline" size="sm" className="px-3 data-[state=on]:bg-background">
      ...
      <CardContent className="flex items-center justify-center p-4">
        {viewMode === 'realtime' ? data ? <div className="w-full h-[500px]">
              {/* Replace D3RealtimeEnergyFlowComponent with VisxRealtimeEnergyFlowComponent */}
              <VisxRealtimeEnergyFlowComponent data={data} className="w-full h-full" configId={configId} />
            ...
            </div> : <div className="w-full h-[500px]">
